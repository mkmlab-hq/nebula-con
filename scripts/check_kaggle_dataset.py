...
def check_kaggle_dataset():
    ...
    try:
        ...
        if accessible_datasets:
            ...
            print(f"\n   💡 권장사항:")
            print(f"      1. 위 데이터셋 중 하나를 선택하여 임베딩 생성")
            print(f"      2. 텍스트 컬럼이 있는 데이터셋 우선 선택")
            print(f"      3. 행 수가 적당한 데이터셋 선택 (1000-10000행)")
        else:
            print(f"   ⚠️ 접근 가능한 공개 데이터셋이 없습니다")
            print(f"   📋 대안:")
            print(f"      1. 자체 테스트 데이터로 계속 진행")
            print(f"      2. 캐글 대회 페이지에서 정확한 데이터셋 경로 확인")
            print(f"      3. BigQuery 콘솔에서 직접 데이터셋 탐색")
