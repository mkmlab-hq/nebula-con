...
# 2. 데이터셋 내 모델 목록 확인 (수정된 쿼리)
try:
    print("\n🔍 데이터셋 'nebula_con_kaggle' 내 모델 목록을 확인합니다...")
    
    # 수정된 쿼리: 프로젝트 ID를 올바르게 참조
    models_query = """
    SELECT 
        model_id,
        model_type,
        creation_time,
        last_modified_time
    FROM `nebula_con_kaggle.INFORMATION_SCHEMA.ML_MODELS`
...
try:
    ...
    SELECT 
        ...
        connection_type,
        creation_time
    FROM `nebula_con_kaggle.INFORMATION_SCHEMA.EXTERNAL_CONNECTIONS`
    ORDER BY creation_time DESC
    """
    
    print("⏳ 연결 목록을 조회합니다...")
    connections_job = client.query(connections_query)
    connections_results = connections_job.result()
    
    print("\n🔗 사용 가능한 연결 목록:")
    connection_count = 0
    for row in connections_results:
        print(f"  - {row.connection_id} (타입: {row.connection_type})")
        connection_count += 1
    
    if connection_count == 0:
        print("  ❌ 사용 가능한 연결이 없습니다.")
        print("\n💡 해결 방법:")
        print("  1. BigQuery 웹 콘솔에서 Vertex AI 연결을 생성해야 합니다.")
        print("  2. 연결 생성 후 원격 모델을 생성해야 합니다.")
    else:
        print(f"\n✅ 총 {connection_count}개의 연결을 찾았습니다.")
        
except Exception as e:
    print(f"\n❌ 연결 목록 조회 중 오류: {e}")
    print("💡 이는 EXTERNAL_CONNECTIONS 스키마에 접근할 수 없음을 의미합니다.") 