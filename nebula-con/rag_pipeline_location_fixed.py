#!/usr/bin/env python3
"""
위치 문제가 해결된 RAG 파이프라인
us-central1 위치의 실제 존재하는 테이블과 데이터셋 사용
"""
...
class RAGPipelineLocationFixed:
    """위치 문제가 해결된 RAG 파이프라인"""
    ...
    def __init__(self, project_id: str = 'persona-diary-service',
        ...
        logger.info(
            f"🚀 위치 문제 해결된 RAG 파이프라인 초기화 완료: "
            f"{project_id}.{dataset_id}"
        )
        logger.info(f"📍 올바른 위치: us-central1")

    def check_existing_tables(self) -> Dict[str, Any]:
        """실제 존재하는 테이블 확인"""
        try:
            tables = list(self.bq_client.list_tables(self.dataset_id))
            
            table_info = {}
            for table in tables:
                table_ref = self.bq_client.dataset(self.dataset_id).table(table.table_id)
...
📍 **위치**: us-central1 (올바른 위치 사용)
    ...
    def run_full_pipeline(self, test_queries: List[str]) -> Dict[str, Any]:
        """전체 RAG 파이프라인 테스트 실행"""
        logger.info("🚀 위치 문제 해결된 RAG 파이프라인 실행 시작!")

        # 1. 테이블 정보 확인
        table_info = self.check_existing_tables()
        logger.info(f"📋 사용 가능한 테이블: {list(table_info.keys())}")
        ...
        # 결과 저장
        output_file = 'rag_pipeline_location_fixed_results.json'
        with open(output_file, 'w', encoding='utf-8') as f:
            json.dump(summary, f, ensure_ascii=False, indent=2)

        logger.info("✅ 위치 문제 해결된 RAG 파이프라인 실행 완료!")
...
def main():
    ...
    dataset_id = "nebula_con_kaggle"  # us-central1에 실제 존재

    # RAG 파이프라인 초기화
    rag_pipeline = RAGPipelineLocationFixed(project_id, dataset_id)
