...
def approximate_dip(x: np.ndarray, grid_size: int = 256) -> Optional[float]:
    ...
    # Calculate average valley depth
    valley_depths = []
    for valley_idx in local_min_indices:
        # Find nearest peaks on both sides
        ...
        if len(left_peaks) > 0 and len(right_peaks) > 0:
            ...
            # Valley depth relative to surrounding peaks
    ...
    # Normalize valley depth
    if global_max > 0:
        normalized_depth = avg_valley_depth / global_max
    else:
        normalized_depth = avg_valley_depth
