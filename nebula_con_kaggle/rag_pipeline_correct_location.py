#!/usr/bin/env python3
"""
올바른 위치(us-central1)를 사용하는 수정된 RAG 파이프라인
실제 프로젝트 구조에 맞게 수정
"""
...
class RAGPipelineCorrectLocation:
    """올바른 위치를 사용하는 RAG 파이프라인"""
    ...
    def __init__(self, project_id: str = 'persona-diary-service', 
        ...
        logger.info(
            f"🚀 올바른 위치 RAG 파이프라인 초기화 완료: {project_id}.{dataset_id}"
        )
    ...
    def search_documents_keyword(self, query_text: str, top_k: int = 5) -> List[Dict[str, Any]]:
        ...
        try:
            # 실제 존재하는 테이블 확인
            if not self.check_table_exists('hacker_news_embeddings_external'):
                logger.error("❌ hacker_news_embeddings_external 테이블이 존재하지 않음")
                return []
...
💡 **BigQuery 활용**: 이 답변은 BigQuery의 키워드 기반 검색을 사용하여 생성되었습니다.
    ...
    def run_full_pipeline(self, test_queries: List[str]) -> Dict[str, Any]:
        """전체 RAG 파이프라인 테스트 실행"""
        logger.info("🚀 올바른 위치 RAG 파이프라인 실행 시작!")
        ...
        # 결과 저장
        output_file = 'rag_pipeline_correct_location_results.json'
        with open(output_file, 'w', encoding='utf-8') as f:
            json.dump(summary, f, ensure_ascii=False, indent=2)
        
        logger.info("✅ 올바른 위치 RAG 파이프라인 실행 완료!")
...
def main():
    ...
    # RAG 파이프라인 초기화
    rag_pipeline = RAGPipelineCorrectLocation(project_id, dataset_id)
